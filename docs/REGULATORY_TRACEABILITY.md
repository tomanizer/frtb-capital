...
| `rfet.py` | RFET-style modellability classification from real-price observation counts and qualitative pass/fail. | MAR31 RFET and real-price evidence for modellability. | Proposed Type A / Type B NMRF distinction and inclusion logic. | Article 325be and Delegated Regulation (EU) 2022/2060. | Simplified counts and dates; no vendor lineage, source deduplication, shifted observation windows, or supervisor overrides. |
| `nmrf.py` | SES aggregation for Type A and Type B NMRFs with audit decomposition. | MAR33 NMRF capital requirement using stress scenarios calibrated at least as prudently as ES. | Proposed SES treatment, including Type A in IMCC and SES, Type B in SES only, Type A zero-correlation aggregation, and Type B rho 0.36. | Article 325bk stress scenario risk measure. | Aggregation formula implemented; SES inputs still come from a linear sensitivity-shock helper. Direct, stepwise, and full revaluation methods are placeholders. |
| `imcc.py` | Unconstrained/constrained IMCC blend and reduced-set stress-scaling helper. | MAR33 IMA capital calculation, current/stress ES, reduced-set scaling concepts. | Proposed IMCC, LHA ES across and within risk classes, and reduced/full set floor at one. | Articles 325ba, 325bb, 325bc. | Returns scalar IMCC today; richer component result is a planned next step. |
| `pla.py` | Kolmogorov-Smirnov PLA comparing HPL and RTPL vectors over the policy window. | MAR32 PLA test requirement for IMA desk eligibility. | Proposed PLA metric uses KS over HPL and RTPL over the most recent 250 business days. | Article 325bg and Delegated Regulation (EU) 2022/2059; EU RTS also uses Spearman and KS. | NPR KS path implemented; EU Spearman remains unsupported. Date-aligned business-calendar trace is not implemented. |
| `backtesting.py` | APL/HPL VaR exception counting over a rolling window, including NPR 97.5% and 99.0% trading-desk gates. | MAR32 backtesting requirements for IMA. | Proposed backtesting over the most recent 250 business days against 97.5% and 99.0% VaR-based measures, including missing-data exception treatment. | Article 325bf and Delegated Regulation (EU) 2022/2059. | Exception gates implemented; no full dated exception trace or business calendar beyond optional official-holiday mask. |
...
| Liquidity horizons | MAR33 10/20/40/60/120 day horizons. | Proposed LHA ES formula and risk-factor liquidity horizons. | Articles 325bc and 325bd. | `LiquidityHorizon`, `liquidity_horizon.py`, `scenario_validation.py`. | Implemented calculation; mapping table not implemented. |
| Risk-factor modellability / RFET | MAR31 risk-factor eligibility test and real-price observations. | Proposed Type A / Type B NMRF working assumptions. | Article 325be and Delegated Regulation (EU) 2022/2060. | `rfet.py`, `RealPriceObservation`, `ModellabilityStatus`. | Simplified prototype. |
| NMRF stress capital / SES | MAR33 NMRF stress scenarios. | Proposed SES treatment for Type A/Type B NMRFs, including Type A zero-correlation aggregation and Type B rho 0.36. | Article 325bk. | `nmrf.py`. | Aggregation implemented; stress-scenario input generation remains simplified. |
| IMCC constrained/unconstrained aggregation | MAR33 IMA capital calculation. | Proposed all-risk-class and per-risk-class LHA ES inputs. | Articles 325ba and 325bb. | `imcc.py`, `liquidity_horizon.py`. | Scalar result implemented; decomposition planned. |
| Reduced-set stress scaling | MAR33 current/stress ES reduced-set concepts. | Proposed reduced/full set scaling floor at one and coverage tests. | Article 325bc subset of modellable risk factors. | `imcc.scale_stress_es`. | Isolated helper only; no reduced-set governance. |
| PLA | MAR32 PLA test. | Proposed KS comparison of RTPL and HPL over 250 business days. | Article 325bg and Delegated Regulation (EU) 2022/2059. | `pla.py`. | NPR KS implemented with policy window; EU Spearman not implemented. |
| Backtesting | MAR32 backtesting. | Proposed APL/HPL backtesting over 250 business days at 97.5% and 99.0% VaR levels. | Article 325bf and Delegated Regulation (EU) 2022/2059. | `backtesting.py`. | Dual-level exception gate implemented; dated trace not implemented. |
