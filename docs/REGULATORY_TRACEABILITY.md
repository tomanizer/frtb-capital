...
| `nmrf.py` | SES aggregation for Type A and Type B NMRFs with audit decomposition and explicit stress-method contracts. | MAR33 NMRF capital requirement using stress scenarios calibrated at least as prudently as ES. | Proposed SES treatment, including Type A in IMCC and SES, Type B in SES only, Type A zero-correlation aggregation, and Type B rho 0.36. | Article 325bk stress scenario risk measure. | Aggregation and external SES input contracts implemented; prototype generation remains limited to a labelled linear sensitivity-shock helper. Direct, stepwise, and full revaluation generation raises unsupported-feature errors. |
...
| NMRF stress capital / SES | MAR33 NMRF stress scenarios. | Proposed SES treatment for Type A/Type B NMRFs, including Type A zero-correlation aggregation and Type B rho 0.36. | Article 325bk. | `nmrf.py`. | Aggregation and stress-method contracts implemented; direct/stepwise/full-revaluation generation not implemented. |
