...
support covers non-securitisation and securitisation non-CTP row and batch
capital. EU CRR3 CTP remains fail-closed with article-level source-map ids for
implementation issue #1003. PRA UK CRR remains fail-closed for all DRC risk
classes; the `PRA_DRC_ARTICLE_*` ids are source-map anchors for issues #1004,
#1005, and #1006, not runtime support evidence. Legacy raw float risk-weight
maps are limited to `US_NPR_2_0` compatibility use.
