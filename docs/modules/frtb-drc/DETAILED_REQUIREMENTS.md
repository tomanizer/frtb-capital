...
enums, and `numpy` where vectorisation is useful. `pandas`, `polars`, `scipy`,
and `statsmodels` may be used in tests, notebooks, validation, research, or
optional adapters only when they do not leak into the core runtime path. Any new
runtime dependency requires an ADR under ADR 0011.
