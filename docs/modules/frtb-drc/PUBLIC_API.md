...
| Handoff specs | `DRC_NONSEC_ARROW_COLUMN_SPECS`, `DRC_SECURITISATION_NON_CTP_ARROW_COLUMN_SPECS`, `DRC_CTP_ARROW_COLUMN_SPECS`, `DRC_RISK_WEIGHT_EVIDENCE_ARROW_COLUMN_SPECS`, `DRC_FAIR_VALUE_CAP_EVIDENCE_ARROW_COLUMN_SPECS` | Client schema alignment and generated schema export. |
| Normalize | `normalize_drc_nonsec_arrow_table`, `normalize_drc_securitisation_non_ctp_arrow_table`, `normalize_drc_ctp_arrow_table`, `normalize_drc_risk_weight_evidence_arrow_table`, `normalize_drc_fair_value_cap_evidence_arrow_table` | Ingress from raw Arrow tables to `NormalizedArrowTable`. |
| Reference overlays | `DrcCalculationContext`, `DrcFxRate`, `DrcRiskWeightEvidence`, `DrcFairValueCapEvidence`, `build_drc_risk_weight_evidence_from_arrow`, `build_drc_securitisation_non_ctp_risk_weight_evidence_from_arrow`, `build_drc_ctp_risk_weight_evidence_from_arrow`, `build_drc_fair_value_cap_evidence_from_arrow` | Run-scoped FX rates, securitisation risk weights, fair-value cap evidence, and offset groups. |
...
| --- | --- | --- | --- |
| 1 - Arrow/Parquet table | One class-specific table per DRC class, plus optional risk-weight and fair-value-cap evidence tables | `normalize_drc_*_arrow_table` -> `build_drc_*_batch_from_arrow`; evidence tables use `normalize_drc_*_evidence_arrow_table` -> `build_drc_*_evidence_from_arrow`; capital runs through `calculate_drc_capital_from_batch` with the resulting `DrcCalculationContext` | Recommended production path **per risk class**; mixed position classes in one table fail closed. Evidence handoffs populate context maps and do not bypass context validation. |
...
## Arrow Evidence Paths

| Evidence attachment | Arrow spec | Normalizer | Builder | Context target |
| --- | --- | --- | --- | --- |
| Securitisation non-CTP risk weights | `DRC_RISK_WEIGHT_EVIDENCE_ARROW_COLUMN_SPECS` | `normalize_drc_risk_weight_evidence_arrow_table` | `build_drc_securitisation_non_ctp_risk_weight_evidence_from_arrow` | `DrcCalculationContext.securitisation_non_ctp_risk_weight_evidence` |
| CTP risk weights | `DRC_RISK_WEIGHT_EVIDENCE_ARROW_COLUMN_SPECS` | `normalize_drc_risk_weight_evidence_arrow_table` | `build_drc_ctp_risk_weight_evidence_from_arrow` | `DrcCalculationContext.ctp_risk_weight_evidence` |
| Fair-value cap evidence | `DRC_FAIR_VALUE_CAP_EVIDENCE_ARROW_COLUMN_SPECS` | `normalize_drc_fair_value_cap_evidence_arrow_table` | `build_drc_fair_value_cap_evidence_from_arrow` | `DrcCalculationContext.securitisation_non_ctp_fair_value_cap_evidence` |

Evidence builders reject duplicate `position_id` records, stale rows, blank
citations, non-finite or negative values, and risk-class mismatches for the
class-specific helpers. Profile mismatch and future-dated evidence are validated
against the run context before capital mechanics execute. Internal banking-book
risk-weight derivation remains out of scope; clients must supply cited evidence
or supported explicit maps.
...
| Bucket and credit attributes | Class-specific | `bucket_key`, `seniority`, and `credit_quality`; securitisation and CTP risk weights are run-scoped context maps or typed evidence attachments. |
...
Evidence Arrow tables use `position_id`, source profile/table/method or cap
eligibility fields, `as_of_date`, lineage source columns, citation ids, stale
flags, and validation flags. Builder output remains deterministic and keyed by
`position_id` for the run context.
