...
`EU_CRR3` cells, and the delivered `PRA_UK_CRR` cells with cited profile-owned
reference data and fixture evidence. Issue #1034 source-maps the remaining
`US_NPR_2_0` CSR cells in [`US_NPR_CSR_MAPPING.md`](US_NPR_CSR_MAPPING.md)
without opening runtime gates; `PRA_UK_CRR` CSR planning is recorded in
[`PRA_UK_CRR_CSR_MAPPING.md`](PRA_UK_CRR_CSR_MAPPING.md). Other non-Basel cells
still fail closed before capital calculation until cited reference data and
...
| `phase1_capital_supported_paths()` | 21 cells (7×3) | `US_NPR_2_0` 8 cells; `EU_CRR3` 8 cells; `PRA_UK_CRR` 8 cells |
| `resolve_sbm_profile()` / `get_sbm_rule_profile()` | Supported | `US_NPR_2_0`, `EU_CRR3`, and `PRA_UK_CRR` support GIRR delta/vega/curvature, FX delta/vega/curvature, equity delta, and commodity delta |
| `PROFILE_*` reference-data maps in `reference_data.py` | Populated | NPR, EU CRR3, and PRA UK CRR GIRR delta/vega/curvature, FX delta/vega/curvature, equity delta, and commodity delta maps populated; other non-Basel lookup paths fail closed |
| Fixture packs under `tests/fixtures/` | 24 non-Basel packs (`*_v1`) | Eight `*_us_npr_v1` packs, eight `*_eu_crr3_v1` fixture packs, and eight `*_pra_uk_crr_v1` fixture packs |
...
- **EU_CRR3** — delivered GIRR, FX, equity delta, and commodity delta cells run
  ...
  cited tables exist.
- **PRA_UK_CRR** — GIRR delta/vega/curvature, FX delta/vega/curvature, equity
  delta, and commodity delta are implemented under audit; CSR and remaining
  cells fail closed until cited tables exist.
...
| `PRA_UK_CRR` | [PRA PS1/26 Appendix 1](https://www.bankofengland.co.uk/-/media/boe/files/prudential-regulation/policy-statement/2026/january/ps126app1.pdf), Market Risk: Advanced Standardised Approach (CRR) Part | Articles 325c-325ay are source-mapped. GIRR delta/vega/curvature, FX delta/vega/curvature, equity delta, and commodity delta are implemented under audit; CSR is mapped in `PRA_UK_CRR_CSR_MAPPING.md`; other cells still fail closed until exact-cell citations, reference data, and fixtures land. |
...
| `PRA_UK_CRR` | 8 / 21 | GIRR delta/vega/curvature, FX delta/vega/curvature, equity delta, and commodity delta implemented under audit; 13 unsupported fail-closed |
...
| FX | Delta, vega, and curvature implemented under audit | Delta, vega, and curvature implemented under audit | Delta, vega, and curvature implemented under audit |
| Equity | Delta implemented under audit; vega/curvature unsupported fail-closed | Delta implemented under audit; vega/curvature unsupported fail-closed | Delta implemented under audit; vega/curvature unsupported fail-closed |
| Commodity | Delta implemented under audit; vega/curvature unsupported fail-closed | Delta implemented under audit; vega/curvature unsupported fail-closed | Delta implemented under audit; vega/curvature unsupported fail-closed |
| CSR non-sec | Source-mapped planned; runtime fail-closed | Planned | Source-mapped in `PRA_UK_CRR_CSR_MAPPING.md`; runtime fail-closed |
| CSR sec non-CTP | Source-mapped planned; runtime fail-closed | Planned | Source-mapped in `PRA_UK_CRR_CSR_MAPPING.md`; runtime fail-closed |
| CSR sec CTP | Source-mapped planned; runtime fail-closed | Planned | Source-mapped in `PRA_UK_CRR_CSR_MAPPING.md`; runtime fail-closed |
...
`PRA_UK_CRR` CSR planning is source-mapped in
[`PRA_UK_CRR_CSR_MAPPING.md`](PRA_UK_CRR_CSR_MAPPING.md); those cells must keep
failing closed until profile-owned PRA reference data and `csr_*_pra_uk_crr_v1`
fixture packs land.
...
| `PRA_UK_CRR_*` | `PRA_UK_CRR_ART_325*_...` | Use PS1/26 Appendix 1 / PRA2026/1 article ids; delivered GIRR, FX, equity delta, and commodity delta cells are implemented, and later cells remain fail-closed until exact-cell citations and fixtures land |
...
| **5** | `PRA_UK_CRR` — GIRR delta/vega/curvature, FX delta/vega/curvature, equity delta, and commodity delta implemented after PS1/26 Appendix 1 mapping; CSR remains source-mapped fail-closed | UK comparison |
...
1. **PRA UK CRR** — CSR cells are source-mapped but runtime fail-closed; open the
   first CSR cell only after exact-cell citations, profile-owned reference data,
   and fixture evidence are added; record PRA-vs-EU divergence in that PR.
