...
`desk_eligibility` is the PLA/backtesting desk eligibility read model for the
Capital Navigator. It is keyed by `run_id` and stable `desk_id`, carries legal
entity, division, business-line, Volcker desk, and book roster fields, and
persists upstream eligibility, PLA, backtesting, RFET/NMRF/SES summary, capital
consequence, profile hash, source hash, timestamp, and artifact references. The
mart stores completed evidence only; it does not calculate PLA, backtesting,
desk approval, RFET, SES, or fallback capital.
