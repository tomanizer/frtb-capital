# Package status dashboard

Generated from [`package_maturity.toml`](package_maturity.toml) and the
regulatory requirement crosswalk files under [`../regulatory/crosswalk/`]
(../regulatory/crosswalk/). Do not edit this file by hand.

Regenerate or verify with:

```bash
make package-status-dashboard
```

| Package | Import | Maturity | Component | Metadata | Entrypoint |
| Required tests | Crosswalk |
| --- | --- | --- | --- | --- | --- | --- | --- |
| `frtb-common` | `frtb_common` | `shared` | `shared` | — | — | `regulatory-helpers` (`packages/frtb-common/tests/test_regulatory_policy_citations.py`) | — |
| `frtb-cva` | `frtb_cva` | `partial_runtime` | `capital` | `frtb_cva:PACKAGE_METADATA` | `frtb_cva:calculate_cva_capital` | `external-comparator` (`packages/frtb-cva/tests/test_cva_external_comparator.py`), `properties` (`packages/frtb-cva/tests/test_cva_properties.py`), `public-api` (`packages/frtb-cva/tests/test_cva_public_api.py`), `sa-cva-fixture-workflow` (`packages/frtb-cva/tests/test_sa_cva_fixture_workflow.py`), `unsupported-runtime-paths` (`packages/frtb-cva/tests/test_cva_unsupported_features.py`) | 7 requirements, 31 source refs ([`frtb-cva.yml`](../regulatory/crosswalk/frtb-cva.yml)) |
| `frtb-drc` | `frtb_drc` | `partial_runtime` | `capital` | `frtb_drc:PACKAGE_METADATA` | `frtb_drc:calculate_drc_capital` | `public-api` (`packages/frtb-drc/tests/test_drc_public_api.py`), `unsupported-runtime-paths` (`packages/frtb-drc/tests/test_drc_public_api.py`) | 2 requirements, 11 source refs ([`frtb-drc.yml`](../regulatory/crosswalk/frtb-drc.yml)) |
| `frtb-ima` | `frtb_ima` | `implemented` | `capital` | `frtb_ima:PACKAGE_METADATA` | `frtb_ima:models_based_capital_for_policy` | `public-api` (`packages/frtb-ima/tests/test_public_api.py`) | 5 requirements, 28 source refs ([`frtb-ima.yml`](../regulatory/crosswalk/frtb-ima.yml)) |
| `frtb-orchestration` | `frtb_orchestration` | `orchestration_partial` | `orchestration` | `frtb_orchestration:PACKAGE_METADATA` | `frtb_orchestration:calculate_suite_capital` | `orchestration-boundary` (`packages/frtb-orchestration/tests/test_orchestration_scaffold.py`) | 2 requirements, 13 source refs ([`frtb-orchestration.yml`](../regulatory/crosswalk/frtb-orchestration.yml)) |
| `frtb-rrao` | `frtb_rrao` | `implemented` | `capital` | `frtb_rrao:PACKAGE_METADATA` | `frtb_rrao:calculate_rrao_capital` | `public-api` (`packages/frtb-rrao/tests/test_public_api.py`) | 2 requirements, 11 source refs ([`frtb-rrao.yml`](../regulatory/crosswalk/frtb-rrao.yml)) |
| `frtb-sbm` | `frtb_sbm` | `partial_runtime` | `capital` | `frtb_sbm:PACKAGE_METADATA` | `frtb_sbm:calculate_sbm_capital` | `public-api` (`packages/frtb-sbm/tests/test_sbm_public_api.py`), `unsupported-runtime-paths` (`packages/frtb-sbm/tests/test_sbm_unsupported_features.py`) | 5 requirements, 36 source refs ([`frtb-sbm.yml`](../regulatory/crosswalk/frtb-sbm.yml)) |

## Maturity legend

| Maturity | Meaning |
| --- | --- |
| `implemented` | Deterministic public calculation path with validation evidence |
| `partial_runtime` | Supported runtime slice with explicit fail-closed paths |
| `orchestration_partial` | Suite handoff contracts without full aggregation |
| `shared` | Shared library, not a capital component |

Outputs from this suite are prototype model-validation evidence, not final
regulatory capital.
