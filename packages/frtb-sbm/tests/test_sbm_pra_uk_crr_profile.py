...
_PRA_PROFILE = "PRA_UK_CRR"
_SUPPORTED_PRA_PATHS = frozenset(
    {
...
def test_pra_uk_crr_support_matrix_opens_only_delivered_cells() -> None:
    assert phase1_capital_supported_paths(_PRA_PROFILE) == _SUPPORTED_PRA_PATHS


@pytest.mark.parametrize(
    ("case_id", "risk_class", "risk_measure"),
    [
        ("girr_delta_pra_uk_crr_v1", SbmRiskClass.GIRR, SbmRiskMeasure.DELTA),
        ("girr_vega_pra_uk_crr_v1", SbmRiskClass.GIRR, SbmRiskMeasure.VEGA),
        ("girr_curvature_pra_uk_crr_v1", SbmRiskClass.GIRR, SbmRiskMeasure.CURVATURE),
        ("fx_delta_pra_uk_crr_v1", SbmRiskClass.FX, SbmRiskMeasure.DELTA),
        ("fx_vega_pra_uk_crr_v1", SbmRiskClass.FX, SbmRiskMeasure.VEGA),
        ("fx_curvature_pra_uk_crr_v1", SbmRiskClass.FX, SbmRiskMeasure.CURVATURE),
        ("equity_delta_pra_uk_crr_v1", SbmRiskClass.EQUITY, SbmRiskMeasure.DELTA),
        ("commodity_delta_pra_uk_crr_v1", SbmRiskClass.COMMODITY, SbmRiskMeasure.DELTA),
    ],
)
def test_pra_uk_crr_supported_fixture_packs_match_expected_outputs(
    case_id: str,
    ...
    assert row_payload["profile_id"] == _PRA_PROFILE
    assert row_payload["total_capital"] > 0.0
    assert not _contains_basel_citation(row_payload)
    assert _contains_pra_citation(row_payload)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "girr_vega_pra_uk_crr_v1",
        "girr_curvature_pra_uk_crr_v1",
        "fx_delta_pra_uk_crr_v1",
        "fx_vega_pra_uk_crr_v1",
        "fx_curvature_pra_uk_crr_v1",
        "equity_delta_pra_uk_crr_v1",
        "commodity_delta_pra_uk_crr_v1",
    ],
)
def test_pra_uk_crr_fixture_manifest_hashes_match_files(case_id: str) -> None:
    manifest = _fixture_manifest(case_id)
    assert manifest["profile"] == _PRA_PROFILE
...
def test_pra_uk_crr_unsupported_cells_remain_fail_closed(
    risk_class: SbmRiskClass,
    risk_measure: SbmRiskMeasure,
) -> None:
    sensitivity = _unsupported_cell_sensitivity(risk_class, risk_measure)
    with pytest.raises(UnsupportedRegulatoryFeatureError):
        calculate_sbm_capital((sensitivity,), context=_context("unsupported-pra-cell"))
...
def _curvature_sensitivity(
    ...
    *,
    ...
    return SbmSensitivity(
        sensitivity_id=f"{risk_class.value.lower()}-curvature-pra",
        source_row_id="row-pra-curvature",
        desk_id="desk-pra",
        legal_entity="entity-pra",
        ...
        lineage=SbmSourceLineage(
            ...
            source_file="pra_uk_crr_curvature_v1",
            source_row_id="row-pra-curvature",
        ),
        qualifier=qualifier,
        tenor=tenor,
...
def _contains_pra_citation(payload: object) -> bool:
    if isinstance(payload, dict):
        return any(_contains_pra_citation(value) for value in payload.values())
    if isinstance(payload, list):
        return any(_contains_pra_citation(value) for value in payload)
    return isinstance(payload, str) and payload.startswith("pra_uk_crr_")
