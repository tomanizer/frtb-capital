...
| Aggregation pattern | Weight sensitivities → intra-bucket `Kb` with correlation scenarios → inter-bucket / risk-class total (delta and vega); curvature uses MAR21.5 branch engine with up/down shocks. Shared aggregation implementation lives under focused `frtb_sbm.kernel.*_aggregation` modules, curvature correlation helpers live under `frtb_sbm.curvature_correlations`, and `frtb_sbm.kernel.aggregation` / `frtb_sbm.aggregation` remain compatibility import paths. |
