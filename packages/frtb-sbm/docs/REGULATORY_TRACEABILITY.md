...
| GIRR delta Arrow/batch handoff | Implemented | #268 — Arrow input converts to `SbmSensitivityBatch`; kernels remain NumPy-native. #269 adds a CRIF-to-Arrow entry path for GIRR delta. #285 generalises the package-owned batch foundation for later homogeneous SBM paths without adding new Arrow capital entrypoints. |
...
| `arrow_handoff.py` | Adapter boundary from normalized Arrow handoff to package-owned GIRR delta batch. | MAR21 risk-factor assignment and weighting context. | Section V.A.7.a tabular input context. | Implemented for GIRR delta (#268); no Arrow in kernels. |
