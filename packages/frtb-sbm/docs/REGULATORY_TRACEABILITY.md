...
Future `PRA_UK_CRR` CSR cells are source-mapped in
[`docs/modules/frtb-sbm/PRA_UK_CRR_CSR_MAPPING.md`](../../../docs/modules/frtb-sbm/PRA_UK_CRR_CSR_MAPPING.md);
that source map likewise does not open any CSR runtime gate.
...
| `PRA_UK_CRR` | partial (8 / 21 cells) | GIRR delta, vega, curvature, FX delta/vega/curvature, equity delta, and commodity delta implemented under audit after the SBM-NBP-020 source map, with profile-owned PRA PS1/26 Appendix 1 / PRA2026/1 citation ids, reference data, and deterministic `*_pra_uk_crr_v1` fixtures; CSR planning is source-mapped in `PRA_UK_CRR_CSR_MAPPING.md`; CSR and non-delivered PRA cells remain unsupported fail-closed. |

`PRA_UK_CRR` GIRR delta, vega, curvature, FX delta/vega/curvature, equity
delta, and commodity delta are runtime-supported only with profile-owned PRA
citation ids and fixture evidence.

Except for the eight delivered `US_NPR_2_0` cells, the eight delivered
`EU_CRR3` cells, and the eight delivered `PRA_UK_CRR` cells, every
...
citation ids. `PRA_UK_CRR` delivered cells are fixture-backed by
`girr_delta_pra_uk_crr_v1`, `girr_vega_pra_uk_crr_v1`,
`girr_curvature_pra_uk_crr_v1`, `fx_delta_pra_uk_crr_v1`,
`fx_vega_pra_uk_crr_v1`, `fx_curvature_pra_uk_crr_v1`,
`equity_delta_pra_uk_crr_v1`, and `commodity_delta_pra_uk_crr_v1`, and carry
PRA profile-owned citation ids. All other entries in this table fail closed
...
| FX | implemented under audit | implemented under audit | implemented under audit | implemented under audit | implemented under audit | implemented under audit | implemented under audit | implemented under audit | implemented under audit |
| Equity | implemented under audit | unsupported fail-closed | unsupported fail-closed | implemented under audit | unsupported fail-closed | unsupported fail-closed | implemented under audit | unsupported fail-closed | unsupported fail-closed |
| Commodity | implemented under audit | unsupported fail-closed | unsupported fail-closed | implemented under audit | unsupported fail-closed | unsupported fail-closed | implemented under audit | unsupported fail-closed | unsupported fail-closed |
...
| PRA UK CRR | PRA PS1/26 Appendix 1 / PRA2026/1, Market Risk: Advanced Standardised Approach (CRR) Part, Articles 325c-325ay. | Partial comparison profile: GIRR delta/vega/curvature, FX delta/vega/curvature, equity delta, and commodity delta implemented under audit; CSR planning is recorded in `PRA_UK_CRR_CSR_MAPPING.md`; CSR and non-delivered PRA UK CRR cells fail closed until exact-cell citations, profile-owned reference data, and fixtures are added. |
...
evidence for `PRA_UK_CRR`. The delivered GIRR delta, vega, curvature, FX
delta/vega/curvature, equity delta, and commodity delta cells carry PRA2026/1
article citations, `PRA_UK_CRR` profile identity, PRA profile hashes, and
deterministic `girr_delta_pra_uk_crr_v1`, `girr_vega_pra_uk_crr_v1`,
`girr_curvature_pra_uk_crr_v1`, `fx_delta_pra_uk_crr_v1`,
`fx_vega_pra_uk_crr_v1`, `fx_curvature_pra_uk_crr_v1`,
`equity_delta_pra_uk_crr_v1`, and `commodity_delta_pra_uk_crr_v1` fixture packs;
future PRA cells must provide the same profile-owned evidence before runtime
gates open. PS1/26 / PRA2026/1 runtime metadata should use the 2027-01-01 effective date.
