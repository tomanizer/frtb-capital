...
| `risk_factor_metadata.py` | Risk-factor evidence view rows that preserve supplied canonical risk-factor identifiers, mapping versions, RFET evidence, NMRF stress evidence, and explicit no-data states for downstream result-store and Navigator views. | MAR31 risk-factor modellability and MAR33 NMRF stress evidence concepts. | Proposed risk-factor evidence, modellability, and NMRF review inputs. | CRR Article 325be and EU RTS 2022/2060 risk-factor modellability concepts. | Projection layer only; does not source market data, classify risk factors, or infer missing evidence. |
...
| `org_scope.py` | Package-local validation and serialization helpers for optional `CalculationScope` metadata carried by IMA inputs, outputs, and evidence records. | Supports MAR31-MAR33 desk-level model governance and audit traceability concepts in consuming modules. | Supports proposed desk/book/model-approval evidence association for hierarchy-aware review. | Supports CRR/FRTB comparison profile governance metadata in consuming modules. | Utility module only; preserves supplied identifiers but does not define enterprise hierarchy traversal, approval workflow, or rollup semantics. |
| `_observation_utils.py` | Compatibility import path for package-local observation-window validation helpers. | Supports existing MAR32 PLA/backtesting diagnostic imports during the validation package split. | Supports existing proposed 250-business-day PLA/backtesting diagnostic imports during the validation package split. | Supports existing EU RTS backtesting and PLA diagnostic imports during the validation package split. | Re-exports `validation/observation_windows.py`; no independent regulatory calculation logic. |
| `validation/observation_windows.py` | Package-local observation-date and window-size validation shared by PLA, backtesting, and stress-period diagnostics. `_observation_utils.py` remains a compatibility import path. | Supports MAR32 dated PLA and backtesting observation-window inputs. | Supports proposed 250-business-day PLA/backtesting observation-window evidence. | Supports EU RTS backtesting and PLA dated-observation concepts. | Utility module only; PLA, backtesting, and stress-period thresholds remain in their consuming modules. |
...
| `stress_periods.py` | Compatibility import surface for stress-period records, rolling-window scoring, policy selection, and NMRF spec bridging. | MAR33 stress-period and NMRF stress-scenario concepts, including common risk-class stress periods. | Proposed-rule basis for stressed ES / SES calibration and Type A / Type B NMRF stress-period inputs. | Article 325bc partial ES and Article 325bk stress scenario risk measure comparison concepts. | Public import path only; physical implementation lives in focused `stress_period_*` modules. |
| `stress_period_types.py` | Immutable stress-period input and candidate records plus shared input validation. | MAR33 stress-period and NMRF stress-scenario input-lineage concepts. | Proposed-rule basis for stressed ES / SES calibration inputs and Type A / Type B NMRF stress-period records. | Article 325bc partial ES and Article 325bk stress scenario risk measure comparison concepts. | Frozen input/candidate records and validation only; does not score windows, select policy stress periods, source market data, or approve calibration. |
| `stress_period_results.py` | Immutable run-level stress-period selection result record and audit serialisation. | MAR33 stress-period and NMRF stress-scenario input-lineage concepts. | Proposed-rule basis for stressed ES / SES calibration result records and Type A / Type B NMRF stress-period inputs. | Article 325bc partial ES and Article 325bk stress scenario risk measure comparison concepts. | Frozen result record only; scoring and policy dispatch live in adjacent stress-period stage modules. |
| `stress_period_windows.py` | Vectorized rolling-window severity scoring and deterministic candidate selection from supplied historical loss vectors. | MAR33 stressed ES / stress-period selection concepts. | Proposed-rule basis for stressed ES / SES calibration from supplied historical severity vectors. | Article 325bc partial ES and Article 325bk stress scenario risk measure comparison concepts. | NumPy scoring stage only; upstream remains responsible for historical scenario generation, pricing, data sourcing, and approval evidence. |
| `stress_period_selection.py` | Run-policy stress-period selection, exact calendar-window handling, logging, validation of selected risk classes, and NMRF stress-period spec bridge. | MAR33 common stress-period and NMRF stress-scenario concepts. | Proposed-rule basis for stressed ES / SES calibration and Type A / Type B NMRF stress-period inputs. | Article 325bc partial ES and Article 325bk stress scenario risk measure comparison concepts. | Applies policy/calendar dispatch and bridges to NMRF specs; raw market-data sourcing, trade pricing, and formal approval evidence remain upstream. |
...
| `validation/rfet_window.py` | RFET observation-window stage for proxy lookback, exact twelve-month business calendars, and shifted-window metadata. | MAR31 RFET and real-price evidence observation-window concepts. | Proposed real-price evidence observation period and shifted-window controls. | Article 325be and Delegated Regulation (EU) 2022/2060 comparison gap. | Row-stage helper only; columnar fast-path window normalization remains in `rfet_evidence.py`. |
| `validation/rfet_thresholds.py` | RFET required-observation thresholds and new-issuance prorating stage. | MAR31 RFET real-price evidence observation-count concepts. | Proposed short/long-liquidity-horizon evidence thresholds and new-issuance treatment. | Article 325be and Delegated Regulation (EU) 2022/2060 comparison gap. | Threshold helper only; evidence assembly and batch assessment remain in `rfet_evidence.py`. |
| `validation/rfet_qualitative.py` | RFET qualitative representativeness validation stage used by audit-grade evidence assessment. | MAR31 RFET and real-price evidence representativeness concepts. | Proposed real-price evidence and qualitative RFET controls. | Article 325be and Delegated Regulation (EU) 2022/2060 comparison gap. | Stage helper only; full RFET evidence assembly and quantitative observation counting remain in `rfet_evidence.py`. |
| `validation/rfet_quantitative.py` | RFET quantitative observation filtering stage with exclusion reasons and accepted date/source/vendor lineage. | MAR31 RFET and real-price evidence observation-count concepts. | Proposed real-price evidence and Type A / Type B NMRF inclusion logic. | Article 325be and Delegated Regulation (EU) 2022/2060 comparison gap. | Row-stage helper only; columnar fast-path assessment remains in `rfet_evidence.py`. |
| `validation/rfet_batch.py` | RFET columnar batch observation-window and required-observation setup stage. | MAR31 RFET real-price evidence observation-window and threshold concepts. | Proposed real-price evidence observation period, short/long-liquidity-horizon thresholds, and new-issuance treatment. | Article 325be and Delegated Regulation (EU) 2022/2060 comparison gap. | Batch setup helper only; public columnar assessment remains in `rfet_evidence.py`. |
| `assembly/rfet.py` | RFET assessment result record, modellability-status selection, and audit count assembly helpers. | MAR31 RFET and real-price evidence result lineage concepts. | Proposed real-price evidence and Type A / Type B NMRF inclusion logic. | Article 325be and Delegated Regulation (EU) 2022/2060 comparison gap. | Assembly helper only; evidence validation and columnar batch assessment remain in `rfet_evidence.py`. |
...
| `nmrf.py` | Compatibility import surface for NMRF record types, stress artifact conversion, SES aggregation, and capital routing. | MAR33 NMRF capital requirement using stress scenarios calibrated at least as prudently as ES. | Proposed SES treatment, including Type A in IMCC and SES, Type B in SES only. | Article 325bk stress scenario risk measure. | Public import path only; physical implementation lives in focused `nmrf_*` modules. |
| `nmrf_types.py` | NMRF stress method enum, stress artifact/result records, capital routing/result records, and SES aggregation result records. | MAR33 NMRF stress-scenario capital and audit-result concepts. | Proposed SES result decomposition and Type A / Type B NMRF record keeping. | Article 325bk stress scenario risk measure. | Frozen records and input-shape validation only; no pricing or capital aggregation. |
| `nmrf_stress.py` | NMRF liquidity-horizon floor, labelled linear approximation helper, unsupported stress-generation gates, and vectorized SES extraction from upstream artifacts. | MAR33 NMRF capital requirement using stress scenarios calibrated at least as prudently as ES. | Proposed NMRF SES methodology and unsupported-feature boundaries for direct/stepwise/full-revaluation generation. | Article 325bk stress scenario risk measure. | Consumes upstream direct/stepwise/full-revaluation artifacts; institutional pricing/revaluation remains upstream. |
| `nmrf_aggregation.py` | Type A zero-correlation SES aggregation, Type B rho aggregation, policy-aware aggregation, and audit decomposition. | MAR33 NMRF capital aggregation concepts. | Proposed Type A zero-correlation aggregation and Type B rho 0.36. | Article 325bk stress scenario risk measure. | Numeric SES aggregation only; does not classify risk factors or validate valuation artifacts. |
| `nmrf_capital.py` | Type A / Type B capital routing, required-artifact validation, and NMRF capital result assembly. | MAR33 NMRF stress-scenario capital and IMCC/NMRF routing concepts. | Proposed SES treatment, including Type A in IMCC and SES, Type B in SES only. | Article 325bk stress scenario risk measure. | Fails hard when Type A/B artifacts are missing or incoherent; delegates SES extraction and aggregation to focused modules. |
...
| `backtesting.py` | Compatibility import surface for APL/HPL VaR exception counting, trading-desk gates, trace records, and policy wrappers. | MAR32 backtesting requirements for IMA. | Proposed backtesting over the most recent 250 business days against 97.5% and 99.0% VaR-based measures, including missing-data exception treatment. | Article 325bf and Delegated Regulation (EU) 2022/2059. | Public import path only; physical implementation lives in focused `backtesting_*` modules. |
| `backtesting_types.py` | Immutable backtesting result and trace records. | MAR32 desk-level backtesting exception concepts. | Proposed APL/HPL backtesting over the most recent 250 business days at 97.5% and 99.0% VaR levels. | Article 325bf and Delegated Regulation (EU) 2022/2059. | Frozen records only; no exception counting or policy dispatch. |
| `backtesting_core.py` | Single-level exception count and scalar backtest result calculation. | MAR32 backtesting exception concepts and MAR99 traffic-light zones. | Proposed APL/HPL backtesting against VaR-based measures. | Article 325bf and Delegated Regulation (EU) 2022/2059. | Scalar NumPy kernel only; trading-desk multi-level traces and policy wrappers live in adjacent modules. |
| `backtesting_trace_inputs.py` | Backtesting trace input validation for aligned APL/HPL/VaR, dates, and official-holiday masks. | MAR32 dated backtesting observation-window concepts. | Proposed 250-business-day APL/HPL backtesting evidence and missing-data treatment. | Article 325bf and Delegated Regulation (EU) 2022/2059. | Input validation helper only; calendar sourcing and exception counting remain outside this helper. |
| `backtesting_trace_levels.py` | Per-confidence-level exception masks and dated observation trace assembly. | MAR32 backtesting exception concepts and MAR99 traffic-light context. | Proposed APL/HPL backtesting at configured VaR levels, including missing-data exception treatment. | Article 325bf and Delegated Regulation (EU) 2022/2059. | Level trace helper only; run-level policy dispatch lives in `backtesting_policy.py`. |
| `backtesting_trace.py` | Trading-desk multi-level backtesting and trace assembly. | MAR32 desk-level backtesting requirements for IMA. | Proposed 97.5% and 99.0% APL/HPL VaR backtesting gates over the policy window. | Article 325bf and Delegated Regulation (EU) 2022/2059. | Combines validated inputs and per-level trace helpers; calendar sourcing remains upstream. |
| `backtesting_policy.py` | Policy-aware backtesting wrappers and run-level logging. | MAR32 backtesting and desk eligibility context. | Proposed section `__.213` backtesting eligibility gates and policy-selected window/threshold parameters. | Article 325bf and Delegated Regulation (EU) 2022/2059. | Applies immutable policy parameters and logging only; does not change exception-counting semantics. |
| `validation/backtesting_stages.py` | Backtesting exception masks, reasons, counts, and traffic-light zone labels. | MAR32 desk-level backtesting exception concepts and MAR99 traffic-light thresholds. | Proposed APL/HPL backtesting over the most recent 250 business days at 97.5% and 99.0% VaR levels. | Article 325bf and Delegated Regulation (EU) 2022/2059. | Stage helper only; public backtesting entrypoints and result records remain in `backtesting.py`. |
...
| `attribution.py` | Desk-level projection from `DeskAuditRecord` capital maps to suite-wide `CapitalContribution` records with input hash propagation, reconciliation status, and explicit residuals for floor/max branches. | MAR33 capital calculation traceability concepts, plus ADR 0038 suite-wide attribution contract. | Proposed models-based measure / non-default capital framework auditability, with ADR 0038 contribution-record requirements. | Article 325ba own-funds requirement under alternative internal models, used only as traceability context for already-computed desk capital. | Projection only; does not recalculate IMCC, SES, PLA add-ons, or desk capital and does not provide analytical Euler decomposition below desk component granularity. |
...
| `arrow_batch.py` | Compatibility import path for IMA Arrow-backed tabular handoff helpers. | Supports existing MAR31-MAR33 input-lineage imports during the adapter package split. | Supports existing proposed-rule input-lineage imports during the adapter package split. | Supports existing CRR input-lineage imports during the adapter package split. | Re-exports `adapters/arrow.py`; no independent regulatory calculation logic. |
| `adapters/arrow.py` | Arrow-backed tabular handoff for capital-run input-manifest artifact lineage, scenario metadata, and RFET observation evidence. | MAR31-MAR33 input lineage, completeness, and model governance concepts. | Proposed market-risk governance, reproducibility, and audit-trail basis. | CRR internal-model governance comparison concepts. | Accepts tabular manifest and evidence inputs only; scenario cubes and scenario-vector kernels remain NumPy-native and upstream risk-engine outputs remain outside this adapter. |
