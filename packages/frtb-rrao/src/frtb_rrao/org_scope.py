"""Organisation-scope metadata helpers for RRAO audit records.

RRAO preserves supplied scope identifiers for downstream result-store rollups,
...
def validate_scope_metadata(scope: object, *, field: str) -> CalculationScope | None:
    ...
    if not isinstance(scope, CalculationScope):
        ...
        raise RraoInputError(f"{field} must be CalculationScope when supplied", field=field)
    return scope
