...
def _risk_factor_snapshot_row(snapshot: RiskFactorMetadataSnapshot) -> dict[str, object]:
    return {
        ...
        "mapping_version": str(snapshot.mapping_version),
...
def _risk_factor_metadata_row(record: RiskFactorMetadataRecord) -> dict[str, object]:
    return {
        ...
        "risk_factor_id": str(record.risk_factor_id),
        "display_name": record.display_name,
        "risk_class": str(record.risk_class),
        "risk_factor_type": str(record.risk_factor_type),
        "mapping_version": str(record.mapping_version),
        "bucket_id": None if record.bucket_id is None else str(record.bucket_id),
        "bucket_label": record.bucket_label,
        "sensitivity_type": None
        if record.sensitivity_type is None
        else str(record.sensitivity_type),
        "currency": None if record.currency is None else str(record.currency),
        "curve_id": record.curve_id,
        "tenor": None if record.tenor is None else str(record.tenor),
        ...
        else str(record.rfet_evidence_id),
...
def _risk_factor_source_mapping_row(mapping: RiskFactorSourceMapping) -> dict[str, object]:
    return {
        ...
        "risk_factor_id": str(mapping.risk_factor_id),
