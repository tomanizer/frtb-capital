...
def build_cva_hedge_batch_from_columns(
    *,
    ...
    -------
    CvaHedgeBatch
        Columnar hedge batch ready for BA-CVA full and SA-CVA hedge recognition paths.
    """
    row_count = len(hedge_ids)
    _require_hedge_column_lengths(row_count, columns=locals())
    batch = CvaHedgeBatch(
        **_hedge_batch_fields(
            row_count=row_count,
...
def _require_hedge_column_lengths(row_count: int, *, columns: Mapping[str, Any]) -> None:
    required = (
        "source_row_ids",
        "counterparty_ids",
        "hedge_types",
    ...
    optional = (
        ...
        "whole_transaction_evidence_ids",
        "market_risk_ima_eligibilities",
    ...
    _require_hedge_lengths(
        row_count,
        required={name: columns[name] for name in required},
        optional={name: columns[name] for name in optional},
    )
