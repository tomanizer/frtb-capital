...
class _NormalizedRow:
    ...
    def source_column(self, field_name: str) -> str | None:
        unique_matches = self.source_columns(field_name)
    ...
    def source_columns(self, field_name: str) -> tuple[str, ...]:
        matches: list[str] = []
        for alias in _FIELD_ALIASES[field_name]:
            matches.extend(self._by_key.get(_field_key(alias), ()))
        return tuple(dict.fromkeys(matches))

    def source_values(self, field_name: str) -> Mapping[str, object]:
        return MappingProxyType(
            {
                source_column: self._row[source_column]
                for source_column in self.source_columns(field_name)
            }
        )
...
def _rejection_from_error(
    ...
    *,
    ...
    rejected_row = DrcRejectedCrifRow(
        ...
        source_values=_source_values_for_error(row, exc.field_names),
    )
...
def _source_columns_for_error(row: _NormalizedRow, field_names: tuple[str, ...]) -> tuple[str, ...]:
    ...
    for field_name in field_names:
        columns.extend(row.source_columns(field_name))
    return tuple(columns)


def _source_values_for_error(
    row: _NormalizedRow,
    field_names: tuple[str, ...],
) -> Mapping[str, object | None]:
    values: dict[str, object | None] = {}
    for field_name in field_names:
        source_values = row.source_values(field_name)
        if len(source_values) > 1:
            values[field_name] = dict(source_values)
        else:
            values[field_name] = next(iter(source_values.values()), None)
    return values
