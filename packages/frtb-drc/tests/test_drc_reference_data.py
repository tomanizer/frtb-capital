...
def test_pra_uk_crr_lgd_table_contains_cited_non_securitisation_values() -> None:
    expected = {
        DrcSeniority.EQUITY: 1.00,
        DrcSeniority.NON_SENIOR_DEBT: 1.00,
        DrcSeniority.SENIOR_DEBT: 0.75,
    ...
    for seniority, expected_lgd in expected.items():
        rule = get_lgd_rule(seniority, profile_id=PRA_UK_CRR_PROFILE_ID)
        assert rule.lgd_rate == expected_lgd
        assert rule.citation_id == "PRA_DRC_ARTICLE_325W"

    with pytest.raises(DrcInputError, match="missing DRC LGD rule"):
        get_lgd_rule(DrcSeniority.PSE, profile_id=PRA_UK_CRR_PROFILE_ID)
...
def test_pra_uk_crr_maturity_policy_is_cited() -> None:
    policy = get_maturity_policy(PRA_UK_CRR_PROFILE_ID)

    assert policy.profile_id == PRA_UK_CRR_PROFILE_ID
    assert policy.floor_years == 0.25
    assert policy.full_weight_years == 1.0
    assert policy.citation_id == "PRA_DRC_ARTICLE_325X"
...
def test_eu_crr3_bucket_definitions_are_cited() -> None:
    ...
    assert all(
        buckets[key].citation_id == "EU_CRR3_ARTICLE_325AA" for key in securitisation_buckets
    )
...
def test_pra_uk_crr_bucket_definitions_are_cited_for_nonsec_and_securitisation() -> None:
    ...
    assert all(
        buckets[key].citation_id == "PRA_DRC_ARTICLE_325AA" for key in securitisation_buckets
    )
...
def test_pra_uk_crr_risk_weight_table_uses_letter_grade_lookup_keys() -> None:
    expected = {
        CreditQuality.AAA: 0.005,
        CreditQuality.AA: 0.02,
        CreditQuality.A: 0.03,
    ...
    for bucket_key in ("CORPORATE", "SOVEREIGN", "LOCAL_GOVERNMENT_MUNICIPAL"):
        for credit_quality, expected_risk_weight in expected.items():
            ...
            assert rule.risk_weight == expected_risk_weight
            assert rule.citation_id == "PRA_DRC_ARTICLE_325Y"
...
def test_pra_uk_crr_reference_data_entries_have_profile_citations() -> None:
    profile = get_rule_profile(PRA_UK_CRR_PROFILE_ID)
    citation_ids = set(profile.citations)

    for rule in iter_lgd_rules(profile_id=PRA_UK_CRR_PROFILE_ID):
        assert rule.citation_id in citation_ids
    for bucket in iter_bucket_definitions(profile_id=PRA_UK_CRR_PROFILE_ID):
        assert bucket.citation_id in citation_ids
    for risk_weight in iter_risk_weight_rules(profile_id=PRA_UK_CRR_PROFILE_ID):
        assert risk_weight.citation_id in citation_ids
    assert get_maturity_policy(PRA_UK_CRR_PROFILE_ID).citation_id in citation_ids
