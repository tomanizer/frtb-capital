...
_FIXTURE_DIR = Path(__file__).resolve().parent / "fixtures" / "drc_pra_nonsec_v1"


def test_pra_uk_crr_nonsec_fixture_matches_expected_stage_outputs() -> None:
...
def test_pra_uk_crr_nonsec_arrow_batch_matches_row_result() -> None:
    fixture = _load_fixture()
    ...
    handoff = normalize_drc_nonsec_arrow_table(
        ...
        source_hash=source_content_hash("synthetic pra drc nonsec source"),
    )

    batch = build_drc_nonsec_batch_from_arrow(handoff, profile_id="PRA_UK_CRR")
    calculation = calculate_drc_capital_from_batch(batch, context=fixture["context"])

    validate_reconciliation(calculation.result)
    assert calculation.result.profile_id == "PRA_UK_CRR"
...
def test_pra_uk_crr_nonsec_fixture_docs_name_each_position_case() -> None:
